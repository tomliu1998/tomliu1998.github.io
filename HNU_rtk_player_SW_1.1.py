...
class RtkPlayer(object):
    ...
    def publish_planningmsg(self):
        ...
        for i in range(self.start, self.end):
            ...
            if self.data['SteerMode'][i] ==8:
                self.planning_pub.write(planningdata)
                self.logger.debug("Generated Planning Sequence: "
                                + str(self.sequence_num - 1))
            
            if self.data['SteerMode'][i] == 2:
                planningdata.tgt_long_speed = 0
                self.planning_pub.write(planningdata)
                time.sleep(2)
                planningdata.tgt_steer_mode_req = 4
                self.planning_pub.write(planningdata)
                time.sleep(10)
                planningdata.tgt_long_speed = 5
                planningdata.tgt_steer_angle_req = 0
                planningdata.shift_pos_req = self.data['gear'][i]
                self.planning_pub.write(planningdata)
