...
class RtkPlayer(object):
    ...
    def cur_planning(self, i_cp_now):
        # num_cur = 0
        # for i_cur_plan in range(i_cp_now, i_cp_now + SEARCH_CUR):
        #     num_cur = num_cur + self.cal_cur(i_cur_plan)
        # cur_planned_aver = num_cur/(SEARCH_CUR + 1)
...
################################################################    四轮转向循迹     ###############################################################################################
            ...
            if (FourWheel_AD_safe == 1 and self.data['gear'][self.i] == 3)or \
            ...
            while((FourWheel_AD_safe == 1 and self.data['gear'][self.i] == 3) or \
                (self.data['SteerMode'][self.i] == 1 and self.chassis_detail.schaeffler.ccu_status_1_208.ccu_actsteermode == 4 and self.data['gear'][self.i] == 3)):
...
####################################################   第五模式切换    ################################################################################################
            ...
            while((Tr_AD_safe == 1 and self.data['gear'][self.i] == 3) or (Tr_AD_safe == 1 and self.data['gear'][self.i] == 1) or\
                ...
                self.planningdata.tgt_steer_angle_req = 0
                #self.planningdata.shift_pos_req = self.data['gear'][self.i]
                self.planningdata.shift_pos_req =  3
                self.planning_pub.write(self.planningdata)

                #while(self.localization.pose.heading == self.data['theta'][(i+self.i_js_5-1)]):
                while(np.absolute(self.cal_theta(self.i+self.i_js_5-1)) <= 10.0):
                    ...
                    if self.chassis.wheel_speed.vehicle_spd == 0:
                        self.i = self.i+self.i_js_5-1
                        self.logger.info("i + i_js_5 - 1: %s", self.i)
                        Tr_AD_safe = 2
                        self.planningdata.brk_pedl_posn_req = 0
                        self.planning_pub.write(self.planningdata)
...
####################################################   第三模式切换    ################################################################################################
            if self.data['SteerMode'][self.i] == 3 and self.chassis_detail.schaeffler.ccu_status_1_208.ccu_actsteermode != 3 and self.data['gear'][self.i] == 3\
                ...
                if self.chassis_detail.schaeffler.ccu_status_1_208.ccu_actsteermode == 3 and self.chassis_detail.schaeffler.ccu_status_1_208.has_ccu_strmodetransstate == 1:
                ...
                self.logger.warning("N_parking_safe: %s" % N_parking_safe)


                

            while((N_parking_safe == 1 and self.data['gear'][self.i] == 3) or (N_parking_safe == 1 and self.data['gear'][self.i] == 1) or\
                ...
                if math.sqrt((self.carx - self.data['x'][self.i_first_N_parking+self.i_js_3-1])**2 + (self.cary - self.data['y'][self.i_first_N_parking+self.i_js_3-1])**2) <= 0.3:
                    ...
                    self.logger.info("N_parking_mode breaking")
                    break
