...
class RtkPlayer(object):
    ...
    def Standby_PD(self):
        ...
        if self.chassis.control_mod ==4:
            ...
            self.logger.warning("being not standby")
    ...
    def Parking_rels(self):
        ...
        if self.chassis.control_mod == 1:
            ...
            self.logger.warning("control mod is not AD")

    def AD_safe(self):
        if(self.chassis.control_mod == 1 and self.chassis.parking_st == 0):
            self.Safe_PD = 1
            self.planningdata.parking_req = 0
            self.planning_pub.write(self.planningdata)
        else:
            self.Safe_PD = 0
            self.logger.warning("being not safe for AD")
        return self.Safe_PD
    ...
    def publish_planningmsg(self):
        ...
        for i in range(self.start, self.end):
            ...
            planningdata.path_point.heading = self.data['theta'][i]
            planningdata.lateral_dis_deviation = self.lateral_dist(self.data['x'], self.data['y'])
            planningdata.tgt_long_speed = self.data['speed'][i]
            planningdata.shift_pos_req = self.data['gear'][i]
            planningdata.tgt_route_curvature = (1+(2*fit_xy[0]*self.data['y'][i]+fit_xy[1])**2)**1.5/2*fit_xy[0]
