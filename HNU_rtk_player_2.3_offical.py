...
class RtkPlayer(object):
    ...
    def cur_planning(self, i_cp_now):
        ...
        cur_planned_now = self.cal_cur(i_cp_now + 100)
        cur_planned_before = self.cal_cur(i_cp_now + 95)

        # if np.absolute(cur_planned_now - cur_planned_before) >= 0.05:
        #     cur_planned_now = cur_planned_before

        if (np.absolute(cur_planned_now - cur_planned_before) >= 0.05) or (cur_planned_now*cur_planned_before)<0:
            cur_planned_now = cur_planned_before
        ...
        cur_planned = cur_planned_now * cur_one
