...
class RtkPlayer(object):
    ...
    def AD_safe(self):
        if(self.chassis_detail.schaeffler.ccu_status_1_208.ccu_ctrlmode == 1 and self.chassis_detail.schaeffler.ccu_status_1_208.ccu_parkingst == 0 and self.chassis_detail.schaeffler.ccu_status_1_208.ccu_gearposst == 3):
            self.Safe_PD = 1
    ...
    def publish_planningmsg(self):
        ...
        for i in range(self.start, self.end):
            ...
            self.planningdata.shift_pos_req = int(self.data['gear'][i])
            fit_xy =  np.polyfit(self.data['y'][i:i+SEARCH_CUR], self.data['x'][i:i+SEARCH_CUR], 2)
            fit_xy_a = (1+(2*fit_xy[0]*self.data['y'][i]+fit_xy[1])**2)**1.5
            fit_xy_b = math.sqrt(2*fit_xy[0])
            if fit_xy_b == 0.0:
                fit_xy_b = 0.0001
            fit_xy_cur = fit_xy_a/fit_xy_b
            #fit_xy_cur = (1+(2*fit_xy[0]*self.data['y'][i]+fit_xy[1])**2)**1.5/math.sqrt(2*fit_xy[0])
            #fit_xy_cur = (1+(2*fit_xy[0]*self.data['y'][i]+fit_xy[1])**2)**1.5/0.001
