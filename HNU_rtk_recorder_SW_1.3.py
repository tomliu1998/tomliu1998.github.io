...
class RtkRecord(object):
    ...
    def __init__(self, record_file):
        ...
        self.write("x,y,z,theta,time,speed,gear,SteerMode,WheelAngle_f,WheelAngle_r\n")
    ...
    def localization_callback(self, data):
        ...
        if math.isnan(self.chassis.wheel_speed.vehicle_spd):
            self.logger.warning("find nan speed_mps: %s" % str(self.chassis))
            return
        '''if math.isnan(self.chassis.steering_percentage):
            self.logger.warning(
        ...
        carspeed = self.chassis.wheel_speed.vehicle_spd
        ...
        #与接口组确定chassis模块获取速度信息的地址
        cargear = self.chassis_detail.schaeffler.ccu_status_1_208.ccu_gearposst
        #获取底盘转向模式
        s_mode = self.chassis_detail.schaeffler.ccu_status_1_208.ccu_actsteermode
        ...
        WA_front = self.chassis.wheel_angle.front_ax_steer_ang
        if math.isnan(self.chassis.wheel_angle.front_ax_steer_ang):
            self.logger.warning("find nan WheelAngle_front" )
            return
        WA_rear = self.chassis.wheel_angle.rear_ax_steer_ang
        if math.isnan(self.chassis.wheel_angle.rear_ax_steer_ang):
            self.logger.warning("find nan WheelAngle_rear" )
            return

        #向文件写入数据
        self.write(
            "%s, %s, %s, %s, %.4f, %s, %s, %s, %s, %s\n" %
            (carx, cary, carz, cartheta, cartime, carspeed, cargear, s_mode, WA_front, WA_rear))
        self.logger.debug(
