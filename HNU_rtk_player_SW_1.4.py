...
class RtkPlayer(object):
    ...
    def closet_point(self, k_cp):
        ...
        for i_mode in range(k_cp, (k_cp+self.i_js_4-1)):
            closet_dist_unsqrt = (self.car_x - self.data['x'][i_mode])**2 + (self.car_y - self.data['y'][i_mode])**2
            S_close_xy = (self.data['x'][i_mode]-self.car_x)*(self.data['y'][i_mode+1]-self.car_y)-(self.data['y'][i_mode]-self.car_y)*(self.data['x'][i_mode+1]-self.car_x)
...
##############################################################################################################################
            if self.data['SteerMode'][i] == 4 and self.chassis_detail.schaeffler.ccu_status_1_208.ccu_actsteermode != 4:
                self.planningdata.brk_pedl_posn_req = 40
                self.planning_pub.write(self.planningdata)
                time.sleep(5)
            ...
            if FourWheel_AD_safe == 1 or (self.data['SteerMode'][i] == 4 and self.chassis_detail.schaeffler.ccu_status_1_208.ccu_actsteermode == 4):
            ...
            while(FourWheel_AD_safe == 1 or (self.data['SteerMode'][i] == 4 and self.chassis_detail.schaeffler.ccu_status_1_208.ccu_actsteermode == 4)):
                ...
                self.planningdata.headingangle_deviation = self.data['theta'][i_now]
